...
abstract class BaseiceModelAttributeValueQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeValuePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeValue A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `STRING`, `NUMERIC`, `BOOLEAN`, `DATE`, `CREATED_AT`, `UPDATED_AT` FROM `attribute_value` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeValue();
      $obj->hydrate($row);
      iceModelAttributeValuePeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
