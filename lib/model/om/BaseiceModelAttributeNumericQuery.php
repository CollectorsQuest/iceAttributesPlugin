...
abstract class BaseiceModelAttributeNumericQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeNumericPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeNumeric A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `VALUE`, `UPDATED_AT`, `CREATED_AT` FROM `attribute_numeric` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeNumeric();
      $obj->hydrate($row);
      iceModelAttributeNumericPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
