...
abstract class BaseiceModelAttributeCategoryQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeCategoryPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeCategory A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `POSITION` FROM `attribute_category` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeCategory();
      $obj->hydrate($row);
      iceModelAttributeCategoryPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
