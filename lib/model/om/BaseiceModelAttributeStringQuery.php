...
abstract class BaseiceModelAttributeStringQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeStringPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeString A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `VALUE`, `UPDATED_AT`, `CREATED_AT` FROM `attribute_string` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeString();
      $obj->hydrate($row);
      iceModelAttributeStringPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
