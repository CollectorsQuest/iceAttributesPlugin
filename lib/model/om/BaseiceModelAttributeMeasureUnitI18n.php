...
abstract class BaseiceModelAttributeMeasureUnitI18n extends BaseObject  implements Persistent
{
  ...
  protected function doInsert(PropelPDO $con)
  {
    ...
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ID`';
    }
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::UNIT))
    {
      $modifiedColumns[':p' . $index++]  = '`UNIT`';
    }
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::NAME))
    {
      $modifiedColumns[':p' . $index++]  = '`NAME`';
    }
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::SLUG))
    {
      $modifiedColumns[':p' . $index++]  = '`SLUG`';
    }
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::DESCRIPTION))
    {
      $modifiedColumns[':p' . $index++]  = '`DESCRIPTION`';
    }
    if ($this->isColumnModified(iceModelAttributeMeasureUnitI18nPeer::CULTURE))
    {
      $modifiedColumns[':p' . $index++]  = '`CULTURE`';
    }

    $sql = sprintf(
      'INSERT INTO `attribute_measure_unit_i18n` (%s) VALUES (%s)',
