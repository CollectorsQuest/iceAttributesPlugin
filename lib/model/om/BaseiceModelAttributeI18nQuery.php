...
abstract class BaseiceModelAttributeI18nQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeI18n A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `NAME`, `NAME_ABBR`, `NAME_TRANSLIT`, `SLUG`, `DESCRIPTION`, `CULTURE` FROM `attribute_i18n` WHERE `ID` = :p0 AND `CULTURE` = :p1';
    try
    {
    ...
    {
      $obj = new iceModelAttributeI18n();
      $obj->hydrate($row);
      iceModelAttributeI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
    }
    $stmt->closeCursor();
