...
abstract class BaseiceModelAttributeDateQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeDatePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeDate A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `VALUE`, `UPDATED_AT`, `CREATED_AT` FROM `attribute_date` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeDate();
      $obj->hydrate($row);
      iceModelAttributeDatePeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
