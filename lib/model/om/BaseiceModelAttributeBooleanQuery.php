...
abstract class BaseiceModelAttributeBooleanQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeBooleanPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeBoolean A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `VALUE`, `UPDATED_AT`, `CREATED_AT` FROM `attribute_boolean` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttributeBoolean();
      $obj->hydrate($row);
      iceModelAttributeBooleanPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
