...
abstract class BaseiceModelAttributeQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttribute A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `MEASURE_UNIT_ID`, `TYPE`, `IS_SPECIAL` FROM `attribute` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelAttribute();
      $obj->hydrate($row);
      iceModelAttributePeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
