...
abstract class BaseiceModelAttributeMeasureUnitI18nQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeMeasureUnitI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeMeasureUnitI18n A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `UNIT`, `NAME`, `SLUG`, `DESCRIPTION`, `CULTURE` FROM `attribute_measure_unit_i18n` WHERE `ID` = :p0 AND `CULTURE` = :p1';
    try
    {
    ...
    {
      $obj = new iceModelAttributeMeasureUnitI18n();
      $obj->hydrate($row);
      iceModelAttributeMeasureUnitI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
    }
    $stmt->closeCursor();
