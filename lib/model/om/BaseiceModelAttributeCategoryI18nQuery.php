...
abstract class BaseiceModelAttributeCategoryI18nQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelAttributeCategoryI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelAttributeCategoryI18n A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `NAME`, `NAME_TRANSLIT`, `SLUG`, `CULTURE` FROM `attribute_category_i18n` WHERE `ID` = :p0 AND `CULTURE` = :p1';
    try
    {
    ...
    {
      $obj = new iceModelAttributeCategoryI18n();
      $obj->hydrate($row);
      iceModelAttributeCategoryI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
    }
    $stmt->closeCursor();
